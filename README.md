...
To accommodate my needs, I had to modify the source code.

- client `signIn` sets needed cookies and sets `credentials: 'include'` to `fetch` options to allow CORS for sessions
- server `callbackUrlHandler` sets `serviceName` from cookie to `req.options`
- email signin helper adds `callbackUrl` and `serviceName` as query params for email handling
